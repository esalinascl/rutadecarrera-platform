...
> **Un solo paquete** con tipos, validación, base de datos y utilidades (decisión D1 del PLAN).
>
> - Fechas: siempre texto ISO 8601 (`IsoDateString`), nunca `Date`.
> - Base de datos: se importa aparte y **solo en el servidor**: `import { crearRepositorios } from '@rcp/shared/db'`.
> - Esquema: la fuente de verdad es `src/db/migrations/001_init_schema.sql`. Ver [docs/database-schema.md](../../docs/database-schema.md).
...
```typescript
import { User, ContextoInicial } from '@rcp/shared';
...
```typescript
import { Conversation } from '@rcp/shared';
...
```typescript
import { Message } from '@rcp/shared';
...
```typescript
import { Analysis } from '@rcp/shared';
...
```typescript
import { ChatRequest, ChatResponse } from '@rcp/shared';
...
// Formatear fechas
const formatted = formatDate('2026-09-24T13:30:00.000Z'); // "24-09-2026, 10:30 a. m." (hora de Santiago)
...
```typescript
import { User, callGemini, validateChatRequest } from '@rcp/shared';
```
...
```bash
# (sin build: el paquete se consume directo desde src/)
